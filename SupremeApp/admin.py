...
class SupremeAdmin(admin.ModelAdmin):
    ...
    def get_search_results(self, request, queryset, search_term):
        ...
        if upload_history:
            uploaded_date = datetime.datetime.strftime(upload_history[0].uploaded_date, '%Y-%m-%d %H:%M:%S')
            queryset = queryset.filter(final_tc_name=request.user).filter(~Q(status="Paid"), date_created__gte = uploaded_date)
        else:
            queryset = queryset.filter(final_tc_name=request.user).filter(~Q(status="Paid"))
