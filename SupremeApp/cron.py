    ...
    for st in Setting.objects.all():
        if lower(now.strftime("%a")) in st.days:
            ...
            def send_backup():
                ...
                file_name = 'SUPREME_DATA_from_%s_to_%s.xlsx' %(datetime.datetime.strftime(date_1 - datetime.timedelta(days=int(st.no_of_backup_days)), '%d-%m-%Y'), datetime.datetime.strftime(datetime.datetime.now(), '%d-%m-%Y'))
